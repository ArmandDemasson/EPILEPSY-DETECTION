...
def create_features(df):
    ...
    for column in channels:
        column_features = {feat: [] for feat in list_of_features}
        for window in df[column]:
            for feat in list_of_features:
                column_features[feat].append(compute_features[feat](window))
        for feat in list_of_features:
            data_to_df[f'{feat}_{column}'] = column_features[feat]
...
def pca_(df):
    ...
    for channel in channels:
        ...
        samples_per_window = channel_data.shape[1]
        channel_data = channel_data.reshape(-1)
        pca_df[channel] = channel_data
    pca_df = pd.DataFrame(pca_df)
    pca = PCA(n_components=2)
    pca_array = pca.fit_transform(pca_df)
    pc1 = pca_array[:, 0].reshape(3114, 30)
    pc2 = pca_array[:, 1].reshape(3114, 30)
    pca_df = pd.DataFrame({
        'PC1': list(pc1),
        'PC2': list(pc2)
    })
