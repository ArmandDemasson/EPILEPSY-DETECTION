...
class EEGGraphDataset(Dataset):
    ...
    def __init__(self, X_ids, dim, path, subjects=[], aug=False):
        ...
        if params.edge_weighting == "dist_matrix":
        ...
        elif params.edge_weighting == 'correlation':
            corr = {}
            for s in subjects:
                ...
                corr[s] = corr_tensor
            self.distances = corr
        elif params.edge_weighting == 'granger_causality':
            gc = {}
            for s in subjects:
                gc_matrix = np.array(self.load_obj('granger_causality_matrices_sub_'+str(s)+'.pkl','/sps/crnl/ademasson/data/Granger_causality/'))
                gc_min, gc_max = np.min(gc_matrix, axis=-1), np.max(gc_matrix, axis=-1)
                gc_matrix = (gc_matrix - gc_min) / (gc_max - gc_min)
                gc_tensor = torch.tensor([gc_matrix[a, b].item() for a, b in product(self.node_ids, self.node_ids)],  dtype=torch.float)
                gc[s] = gc_tensor
            self.distances = gc

        if params.edge_filter is not None:
            mask = self.distances > params.edge_filter
